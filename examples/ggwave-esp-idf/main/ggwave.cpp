...
extern "C" ggwave_Parameters ggwave_getDefaultParameters(void)
{
    return GGWave::getDefaultParameters();
}

extern "C" ggwave_Instance ggwave_init(const ggwave_Parameters parameters)
{
    static ggwave_Instance curId = 0;

    g_instances[curId] = new GGWave({parameters.payloadLength,
...
extern "C" void ggwave_free(ggwave_Instance instance)
{
    delete (GGWave *)g_instances[instance];
    g_instances.erase(instance);
}

extern "C" int ggwave_encode(
...
namespace
{
        ...
        float scalar, int startId, int finalId, int cycleMod, int nPerCycle)
    {
        ...
        for (int i = startId; i < finalId; i++)
        {
            ...
            if (k < nBegin)
            {
                dst->at(i) += scalar * src->at(i) * (k * ids);
            }
            else if (k > nEnd)
            {
                dst->at(i) += scalar * src->at(i) * (((float)(nTotal)-k) * ids);
            }
            else
            {
                dst->at(i) += scalar * src->at(i);
            }
        }
    }
    ...
    int bytesForSampleFormat(GGWave::SampleFormat sampleFormat)
    {
        switch (sampleFormat)
        {
        case GGWAVE_SAMPLE_FORMAT_UNDEFINED:
            return 0;
            break;
...
GGWave::GGWave(const Parameters &parameters) : m_sampleRateInp(parameters.sampleRateInp),
                                               ...
                                               m_fftInp(new std::vector<float>(kMaxSamplesPerFrame)),
                                               m_fftOut(new std::vector<float>(2 * kMaxSamplesPerFrame)),
                                               m_hasNewSpectrum(false),
                                               m_hasNewAmplitude(false),
                                               m_sampleSpectrum(kMaxSamplesPerFrame),
                                               m_sampleAmplitude(
                                                   kMaxSamplesPerFrame + 128),                      // small extra space because sometimes resampling needs a few more samples
                                               ...
                                               m_rxData(new TxRxData(kMaxDataSize)),
                                               ...
                                               m_sampleAmplitudeHistory(0), //kMaxSpectrumHistory),
                                               m_historyIdFixed(0),
                                               // Tx
                                               m_hasNewTxData(false),
                                               m_sendVolume(0.1),
                                               m_txDataLength(0),
                                               m_txData(new TxRxData(kMaxDataSize)),
                                               m_txDataEncoded(new TxRxData(kMaxDataSize)),
                                               m_outputBlock(new AmplitudeData(kMaxSamplesPerFrame)),
                                               //        m_outputBlockResampled(new AmplitudeData(2 * kMaxSamplesPerFrame)),
                                               m_outputBlockTmp(new TxRxData(kMaxRecordedFrames * kMaxSamplesPerFrame *
                                                                             m_sampleSizeBytesOut)), // kMaxRecordedFrames*kMaxSamplesPerFrame*m_sampleSizeBytesOut),
                                               m_outputBlockI16(new AmplitudeDataI16(
                                                   kMaxRecordedFrames * kMaxSamplesPerFrame)), //kMaxRecordedFrames*kMaxSamplesPerFrame),
                                               m_impl(new Impl())
{
...
GGWave::~GGWave()
{
    ...
    delete m_rxData;
    delete m_txData;
    delete m_txDataEncoded;
    delete m_outputBlock;
    //    delete m_outputBlockResampled;
    delete m_outputBlockTmp;
    delete m_outputBlockI16;
    delete m_txAmplitudeDataI16;
    m_impl.reset();
}
...
bool GGWave::init(int dataSize, const char *dataBuffer, const TxProtocol &txProtocol, const int volume)
{
    ...
    if (m_txDataLength > 0)
    {
        m_txData->at(0) = m_txDataLength;
        for (int i = 0; i < m_txDataLength; ++i)
            m_txData->at(i + 1) = text[i];
    ...
    {
        m_fftOut->at(2 * i + 0) = 0.0f;
        m_fftOut->at(2 * i + 1) = 0.0f;
    }
...
uint32_t GGWave::encodeSize_samples() const
{
    ...
    int totalDataFrames =
        ((totalBytes + m_txProtocol.bytesPerTx - 1) / m_txProtocol.bytesPerTx) * m_txProtocol.framesPerTx;
...
bool GGWave::encode(const CBWaveformOut &cbWaveformOut)
{
    ...
    auto *phaseOffsets = new std::vector<double>(kMaxDataBits);
    ...
    auto *dataBits = new std::vector<bool>(kMaxDataBits);

    auto *bit1Amplitude = new std::vector<AmplitudeData *>();
    auto *bit0Amplitude = new std::vector<AmplitudeData *>();
    for (int k = 0; k < (int)dataBits->size(); ++k)
    {
        double freq = bitFreq(m_txProtocol, k);

        bit1Amplitude->push_back(new std::vector<float>(kMaxSamplesPerFrame));
        bit0Amplitude->push_back(new std::vector<float>(kMaxSamplesPerFrame));
        ...
        {
            double curi = i;
            bit1Amplitude->at(k)->at(i) =
                (2.0 * M_PI) * (curi * m_isamplesPerFrame) * (freq * curIHzPerSample) + phaseOffset;
        }
        for (int i = 0; i < m_samplesPerFrame; i++)
        {
            double curi = i;
            bit0Amplitude->at(k)->at(i) = std::sin((2.0 * M_PI) * (curi * m_isamplesPerFrame) *
                                                       ((freq + m_hzPerSample * m_freqDelta_bin) * curIHzPerSample) +
                                                   phaseOffset);
        }
    }
    delete phaseOffsets;
    ...
    while (m_hasNewTxData)
    {
        ...
        if (frameId < m_nMarkerFrames)
        {
            ...
            for (int i = 0; i < m_nBitsInMarker; ++i)
            {
                if (i % 2 == 0)
                {
                    ::addAmplitudeSmooth(bit1Amplitude->at(i), m_outputBlock, m_sendVolume, 0, m_samplesPerFrame,
                                         frameId,
                                         m_nMarkerFrames);
                }
                else
                {
                    ::addAmplitudeSmooth(bit0Amplitude->at(i), m_outputBlock, m_sendVolume, 0, m_samplesPerFrame,
                                         frameId,
                                         m_nMarkerFrames);
                }
            }
        ...
        else if (frameId < m_nMarkerFrames + totalDataFrames)
        {
            ...
            {
                {
                    uint8_t d = m_txDataEncoded->at(dataOffset + j) & 15;
                    (*dataBits)[(2 * j + 0) * 16 + d] = 1;
                }
                {
                    uint8_t d = m_txDataEncoded->at(dataOffset + j) & 240;
            ...
            for (int k = 0; k < 2 * m_txProtocol.bytesPerTx * 16; ++k)
            {
                ...
                if (k % 2)
                {
                    ::addAmplitudeSmooth(bit0Amplitude->at(k / 2), m_outputBlock, m_sendVolume, 0, m_samplesPerFrame,
                                         cycleModMain, m_txProtocol.framesPerTx);
                }
                else
                {
                    ::addAmplitudeSmooth(bit1Amplitude->at(k / 2), m_outputBlock, m_sendVolume, 0, m_samplesPerFrame,
                                         cycleModMain, m_txProtocol.framesPerTx);
                }
            }
        }
        else if (frameId < m_nMarkerFrames + totalDataFrames + m_nMarkerFrames)
        {
            ...
            for (int i = 0; i < m_nBitsInMarker; ++i)
            {
                if (i % 2 == 0)
                {
                    addAmplitudeSmooth(bit0Amplitude->at(i), m_outputBlock, m_sendVolume, 0, m_samplesPerFrame, fId,
                                       m_nMarkerFrames);
                }
                else
                {
                    addAmplitudeSmooth(bit1Amplitude->at(i), m_outputBlock, m_sendVolume, 0, m_samplesPerFrame, fId,
                                       m_nMarkerFrames);
                }
            }
        }
        ...
        for (int i = 0; i < m_samplesPerFrame; ++i)
        {
            m_outputBlock->at(i) *= scale;
        }

        int samplesPerFrameOut = m_samplesPerFrame;
        if (m_sampleRateOut != kBaseSampleRate)
        {
            samplesPerFrameOut = m_impl->resampler.resample(factor, m_samplesPerFrame, m_outputBlock->data(),
                                                            m_outputBlockResampled->data());
        }
        else
        {
        ...
        {
            m_outputBlockI16->at(offset + i) = 32768 * m_outputBlockResampled->at(i);
        }
        ...
        case GGWAVE_SAMPLE_FORMAT_U8:
        {
            ...
            for (int i = 0; i < samplesPerFrameOut; ++i)
            {
                p[offset + i] = 128 * (m_outputBlockResampled->at(i) + 1.0f);
            }
        }
        break;
        case GGWAVE_SAMPLE_FORMAT_I8:
        {
            ...
            for (int i = 0; i < samplesPerFrameOut; ++i)
            {
                p[offset + i] = 128 * m_outputBlockResampled->at(i);
            }
        }
        break;
        case GGWAVE_SAMPLE_FORMAT_U16:
        {
            ...
            for (int i = 0; i < samplesPerFrameOut; ++i)
            {
                p[offset + i] = 32768 * (m_outputBlockResampled->at(i) + 1.0f);
            }
        }
        break;
        case GGWAVE_SAMPLE_FORMAT_I16:
        ...
        case GGWAVE_SAMPLE_FORMAT_F32:
        {
            ...
            for (int i = 0; i < samplesPerFrameOut; ++i)
            {
                p[offset + i] = m_outputBlockResampled->at(i);
            }
        }
        break;
        }
    ...
    m_txAmplitudeDataI16 = new AmplitudeDataI16(offset);
    for (uint32_t i = 0; i < offset; ++i)
    {
        m_txAmplitudeDataI16->at(i) = m_outputBlockI16->at(i);
    }
    delete dataBits;

    for (auto &i : *bit1Amplitude)
    {
        delete i;
    }
    delete bit1Amplitude;
    for (auto &i : *bit0Amplitude)
    {
        delete i;
    }
    delete bit0Amplitude;
...
void GGWave::decode(const CBWaveformInp &cbWaveformInp)
{
    while (m_hasNewTxData == false)
    {
        ...
        if (m_sampleRateInp != kBaseSampleRate)
        {
            ...
            nBytesNeeded =
                (m_impl->resampler.resample(1.0f / factor, m_samplesNeeded, m_sampleAmplitudeResampled.data(),
                                            nullptr) +
                 4) *
        ...
        if (m_sampleRateInp != kBaseSampleRate)
        {
            ...
            int nSamplesResampled = offset + m_impl->resampler.resample(factor, nSamplesRecorded,
                                                                        m_sampleAmplitudeResampled.data(),
                                                                        m_sampleAmplitude.data() + offset);
...
void GGWave::decode_variable()
{
    ...
    if (m_historyId == 0 || m_receivingData)
    {
        ...
        for (int i = 0; i < m_samplesPerFrame; ++i)
        {
            m_sampleSpectrum[i] = (m_fftOut->at(2 * i + 0) * m_fftOut->at(2 * i + 0) +
                                   m_fftOut->at(2 * i + 1) * m_fftOut->at(2 * i + 1));
        }
        ...
        for (const auto &rxProtocolPair : m_rxProtocols)
        {
            ...
            for (int ii = m_nMarkerFrames * stepsPerFrame - 1; ii >= 0; --ii)
            {
                ...
                for (int itx = 0; itx < 1024; ++itx)
                {
                    ...
                    if (offsetTx >= m_recvDuration_frames * stepsPerFrame ||
                        (itx + 1) * rxProtocol.bytesPerTx >= (int)m_txDataEncoded->size())
                    {
                        break;
                    }
                    ...
                    for (int k = 1; k < rxProtocol.framesPerTx; ++k)
                    {
                        for (int i = 0; i < m_samplesPerFrame; ++i)
                        {
                            m_fftInp->at(i) += m_recordedAmplitude[(offsetTx + k * stepsPerFrame) * step + i];
                        }
                    }
                    ...
                    for (int i = 0; i < 2 * rxProtocol.bytesPerTx; ++i)
                    {
                        ...
                        {
                            curByte += (kmax << 4);
                            m_txDataEncoded->at(itx * rxProtocol.bytesPerTx + i / 2) = curByte;
                            curByte = 0;
                        }
                        else
                        {
                    ...
                    if (itx * rxProtocol.bytesPerTx > m_encodedDataOffset && knownLength == false)
                    {
                        ...
                        if ((rsLength.Decode(m_txDataEncoded->data(), m_rxData->data()) == 0) &&
                            (m_rxData->at(0) > 0 && m_rxData->at(0) <= 140))
                        {
                            knownLength = true;
                            decodedLength = m_rxData->at(0);

                            const int nTotalBytesExpected =
                                m_encodedDataOffset + decodedLength + ::getECCBytesForLength(decodedLength);
                            const int nTotalFramesExpected = 2 * m_nMarkerFrames +
                                                             ((nTotalBytesExpected + rxProtocol.bytesPerTx - 1) /
                                                              rxProtocol.bytesPerTx) *
                                                                 rxProtocol.framesPerTx;
    ...
    if (m_receivingData == false)
    {
        ...
        for (const auto &rxProtocol : getTxProtocols())
        {
            ...
            for (int i = 0; i < m_nBitsInMarker; ++i)
            {
                ...
                if (i % 2 == 0)
                {
                    if (m_sampleSpectrum[bin] <= m_soundMarkerThreshold * m_sampleSpectrum[bin + m_freqDelta_bin])
                        --nDetectedMarkerBits;
                }
                else
                {
        ...
        if (isReceiving)
        {
            ...
            m_recvDuration_frames =
                2 * m_nMarkerFrames +
                maxFramesPerTx() *
                    ((kMaxLengthVarible + ::getECCBytesForLength(kMaxLengthVarible)) / minBytesPerTx() + 1);
    ...
    else
    {
        ...
        for (const auto &rxProtocol : getTxProtocols())
        {
            ...
            for (int i = 0; i < m_nBitsInMarker; ++i)
            {
                ...
                if (i % 2 == 0)
                {
                    if (m_sampleSpectrum[bin] >=
                        m_soundMarkerThreshold * m_sampleSpectrum[bin + m_freqDelta_bin])
                        nDetectedMarkerBits--;
                }
                else
                {
...
void GGWave::decode_fixed()
{
    ...
    for (const auto &rxProtocolPair : m_rxProtocols)
    {
            ...
            for (int i = 0; i < rxProtocol.framesPerTx; ++i)
            {
                ...
                for (int j = 0; j < rxProtocol.bytesPerTx; ++j)
                {
                    ...
                    for (int b = 0; b < 16; ++b)
                    {
                        ...
                        {
                            const auto &v = m_spectrumHistoryFixed[historyId][binStart + 2 * j * binDelta + binDelta +
                                                                              b];
        ...
        if (detectedSignal)
        {
            ...
            {
                m_txDataEncoded->at(j) = (detectedBins[2 * j + 1] << 4) + detectedBins[2 * j + 0];
            }
